...
项目包含一套本地自动开发体系：Planner Agent 每 24 小时只读审计项目并创建候选 Issues，低风险任务自动进入队列；Dev Agent 调用 Codex 开发、测试并创建 PR。管理页只监听 `127.0.0.1:8787`，合并需要质量门禁和两次人工确认。

首次配置、启动和安全说明见 [docs/DEV_AGENT.md](docs/DEV_AGENT.md)。
