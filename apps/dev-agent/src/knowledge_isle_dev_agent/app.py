...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:"IBM Plex Mono","Cascadia Code",monospace}
    ...
    @media(max-width:850px){.shell{grid-template-columns:minmax(0,1fr);overflow-x:hidden}.rail{border-right:0;border-bottom:2px solid;flex-direction:row}.lamp{align-self:center}main{min-width:0;padding:26px 24px}header{align-items:flex-start;flex-direction:column;gap:20px}header button{align-self:flex-start}.meta{flex-wrap:wrap;gap:10px 16px}.planner{grid-template-columns:minmax(0,1fr);padding:14px}.planner button{justify-self:start}.grid{grid-template-columns:minmax(0,1fr)}h1{font-size:48px}}
  </style>
</head>
